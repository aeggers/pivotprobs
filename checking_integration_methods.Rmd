...
condorcet_sims <- gtools::rdirichlet(10000000, alpha_condorcet)
condorcet_dmc <- condorcet_pivot_probs_from_sims(condorcet_sims)
condorcet_dmc 
```

The function also computes Kemeny pivot probabil
...
condorcet_win_conditions <- rbind(
  ...
  c(1,-1,1,1,-1,-1)  # b beats c -- not actually a requirement for a to win, but this is a requirement for the a-b contest to be decisive
)
...
The initial results with 1M simulations had the right order and similar magnitudes but substantial discrepancies. Expanding it to 10M made no difference, so we seem to have a genuine discrepancy. 
