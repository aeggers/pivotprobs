...
- consider putting ordinal flag in event_list. 
...
```{r check_simulation_approach}
# xxxx
el <- plurality_event_list(n = 1000, k = 4)
sim_4 <- event_probabilities_from_event_list(event_list = el, ordinal = F, alpha = c(10, 7, 5, 4), draw_sims = T, cand_names = c("a", "b", "c", "d"))
```
...
```{r check_simulation_approach_borda}
# xxxx
borda <- positional_event_list(n = 1000, s = .5)
sim_borda <- event_probabilities_from_event_list(event_list = borda, alpha = c(10, 2, 5, 4, 3, 7), draw_sims = T, cand_names = c("a", "b", "c"))
```
...
<!-- This has to await IRV fixing.  -->
```{r check_simulation_approach_irv}
# xxxx
irv <- irv_event_list(n = 1000, s = .5)
sim_irv <- event_probabilities_from_event_list(event_list = irv, alpha = c(10, 2, 9, 2, 7, 8), draw_sims = T, cand_names = c("a", "b", "c"))
```
...
compare_results(sim_irv, sim_irv)
```
