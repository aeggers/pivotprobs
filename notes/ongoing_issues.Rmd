...
- define adjacent events in the spec and update cod to do this
...
n <- 1000
el[["i_"]] <- list(
  conditions = rbind(c(1, -1, 0, 0, 1/n),    # i beats j
                      c(1, 0, -1, 0, 1/n),   # i beats k
                      c(1, 0, 0, -1, 1/n)),  # i beats l
  rows_to_alter = c(),
  scaling_factor = 1,
  P = rbind(rep(1, 4), 0, 0, 0)
)
...
el[["i_j"]] <- list(
  ...
  P = rbind(c(1,0,1,1), c(0,1,0,0), 0, 0)
)
...
el[["i_jk"]] <- list(
  ...
  P = rbind(c(1,0,0,1), c(0,1,0,0), c(0,0,1,0), 0)
)

```

For plurality, we would want to develop some methods for generating these conditions for $k$ candidates -- that's 10 minutes work. 

For a positional method with 3 candidates: 
...
borda_events <- positional_event_list()

```

For IRV with 3 candidates: 
...
irv_events[["i_j"]] <- list(
  ...
  scaling_factor = 1, # not sure about this -- will have to find out
...
OK then what do we do with these events lists? 

The idea is to cycle through all orderings of candidates and get all of these probabilities. We can use chunks from what we have already.

How would this look? 

```{r cycle_func}
# see function event_probabilities_from_event_list()
```

```{r try_it_basic, cache = T}
irv_out <- event_probabilities_from_event_list(event_list = irv_events, alpha = rep(5, 6), cand_names = c("a", "b", "c"))

plurality_out <- event_probabilities_from_event_list(event_list = el, alpha = c(10, 9, 6, 4), cand_names = c("a", "b", "c", "d"), ordinal = F)

borda_out <- event_probabilities_from_event_list(event_list = borda_events, alpha = rep(5, 6), cand_names = c("a", "b", "c"), ordinal = T)
```


```{r try_it_drop_skip, cache = T}
irv_out <- event_probabilities_from_event_list(event_list = irv_events, alpha = rep(5, 6), cand_names = c("a", "b", "c"), drop_dimension = T, skip_non_pivot_events = T)

plurality_out <- event_probabilities_from_event_list(event_list = el, alpha = c(10, 9, 6, 4), cand_names = c("a", "b", "c", "d"), ordinal = F, drop_dimension = T, skip_non_pivot_events = T)

borda_out <- event_probabilities_from_event_list(event_list = borda_events, alpha = rep(5, 6), cand_names = c("a", "b", "c"), ordinal = T, drop_dimension = T, skip_non_pivot_events = T)
```
