...
<!--  \end{pmatrix}. -->
<!-- \] -->
Then we can combine the event-specific election probability matrices to create the *election probability matrix* $\mathbf{P}$  
...
The first step is to convert the set of inequalities defining $E$ into the vertices of the convex hull of the election event, i.e. the points at the "corners" of the region where the event takes place. (We use the `rcdd::scdd()` function in `R`, which is an implementation of the C library `cddlib`.) Next we apply a Delaunay triangulation to divide the region defined by $E$ into disjoint simplices. For example, if $B=3$ we divide $E$ into triangles; if $B = 4$ we divides $E$ into tetrahedrons. (We use the `geometry::delaunayn()` function in `R`, which interfaces with the Qhull library: TODO cite.)  Finally, we apply an adaptive integration algorithm by Genz and Cools (TODO: cite) which integrates the function on each simplex, efficiently allocating computational power by using a finer mesh on simplices where the integrand is less flat. (We use the `SimplicialCubature` package by John P. Nolan.) 

Pick up here. Example? Table? 
