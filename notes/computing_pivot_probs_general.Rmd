...
Typically scholars look for ways to simplify the problem, either for conceptual clarity or computational efficiency: focus on pivot events (i.e. don't compute the probability of each candidate being elected) because other events don't affect the vote (everyone); focus on three candidates (Fisher, Myatt); ignore pivot events involving more than one tie because they are sufficiently unlikely to not matter (e.g. Hoffman, Fisher-Myatt); focus only on relative pivot probabilities (Fisher-Myatt, Hoffman); assume that one candidate is just as likely to be one vote behind or ahead, i.e. that adjacent pivot events are equally likely (everyone except swing voter's curse and underdog effect lit); assume convenient distributions for beliefs (Fisher-Myatt, Eggers-Vivyan, Eggers-Nowacki). 

For expositional purposes there is no doubt that it can be useful to simplify the problem. Also, there are circumstances where it is important to compute probabilities quickly (e.g. when many scenarios must be analyzed), and shortcuts like ignoring three-way ties or merging adjacent pivot events may be harmless in particular applications. But there are also some costs to making restrictive assumptions that simplify the computation of election probabilities. Most obviously, there are cases where it is not realistic to assume three candidates or posit Dirichlet beliefs; also, we would like methods that extend to types of elections beyond plurality. For some purposes (such as measuring the efficiency of a voting system) it is useful to compute the probability of each candidate being elected, in which case a method that computes only pivot events is not ideal. Computing absolute probabilities of pivot events rather than just relative probabilities is useful for generating comparable measures of the incentive to vote or to vote strategically. Separately computing the probability of adjacent pivot events rather than assuming that they are equal may be useful given that the literature on the swing voter's curse and the underdog effect are based on the assumption that differences in adjacent pivot probabilities noticeably affect voter behavior. In many cases ignoring three-way ties and other compound pivot events will make little difference, but this depends on assumptions about the distribution over election outcomes, and in any case we might as well know these probabilities. 

More generally, starting from the exhaustive approach (with no shortcuts or approximations) helps clarify conceptually what is at stake. If we can precisely compute the probability of all potentially relevant election events, then we can check whether approximations we make for computational efficiency or conceptual simplicity are of any practical consequence. Notably, although computational efficiency is one of the motivations for previous methods for computing pivot probabilities, we cannot assess the efficiency of any particular method without a benchmark. (MORE HERE.) 
...
**Definition** Two election outcomes $\mathbf{v}'$ and $\mathbf{v}'$ are part of the same election event $E$ if[^ifandonlyif] $p_{i,j}(\mathbf{v'}) = p_{i,j}(\mathbf{v''}) = p_{i,j}(E) \, \forall \, i,j$.

[^ifandonlyif]: We could say "if and only if", but there are times when it may be convenient to distinguish between subsets of election outcomes with the same event-specific probability matrix, e.g. because there are multiple ways for a candidate to win, with the union of the corresponding spaces not being convex.  
...
<!--  \end{pmatrix}. -->
<!-- \] -->
We can combine the event-specific election probability matrices to create the *election probability matrix* $\mathbf{P}$  
...
### Enumerating election events 

Constructing the $\mathbf{P}$ matrix requires first partitioning the set of possible results into election events, i.e. sets of election results where an additional ballot has a homogenous effect on the outcome. How do we identify the relevant election events in a given voting system?  

Plurality elections are the most straightforward case. Given three candidates $a$, $b$, and $c$, and focusing on election outcomes where candidate $a$ has the highest vote share,[^no_ties] there are four election events depending on whether $a$'s lead over $b$ is more or less than $\frac{1}{n}$ and whether $a$'s lead over $c$ is more or less than $\frac{1}{n}$. The table below shows the conditions. At event $a$ candidate $a$ wins regardless of an additional ballot; at event $ab$ candidate $a$ wins unless $b$ gains an additional ballot, in which case $b$ wins; at event $ac$ candidate $a$ wins unless $c$ gains an additional ballot, in which case $c$ wins; at event $abc$ either $b$ or $c$ can win by gaining an additional ballot. 

\begin{tabular}{cc}
Election event & Conditions \\
$a$ & $v_a - v_b > \frac{1}{n}$,  $v_a - v_c > \frac{1}{n}$, $v_i \in [0,1] \, \forall \, i$, $\sum_i v_i = 1$ \\ 
$ab$ & $v_a - v_b > 0$, $v_a - v_b < \frac{1}{n}$, $v_a - v_c > \frac{1}{n}$, $v_i \in [0,1] \, \forall \, i$, $\sum_i v_i = 1$ \\
$ac$ & $v_a - v_b > \frac{1}{n}$, $v_a - v_c > 0$,  $v_a - v_c < \frac{1}{n}$, $v_i \in [0,1] \, \forall \, i$, $\sum_i v_i = 1$ \\
$abc$ & $v_a - v_b > 0$, $v_a - v_b < \frac{1}{n}$, $v_a - v_c > 0$,  $v_a - v_c < \frac{1}{n}$, $v_i \in [0,1] \, \forall \, i$, $\sum_i v_i = 1$ \\
\end{tabular} 

<!-- If no to both, a single voter cannot affect the outcome; if yes to both, a single ballot could change the winner from $a$ to either $b$ or $c$.  -->

In total (i.e. including cases where $b$ or $c$ is in the lead), there are twelve election events --- one non-pivot event for each candidate (e.g. where $a$ wins by more than one vote), two near-ties for for each pair of candidates (e.g. where $a$ is just ahead of $b$ and one where $b$ is just ahead of $a$), and three three-way near-ties (e.g. where $a$ is just ahead of both $b$ and $c$). More generally, there are $K \sum_{m=0}^{K-1} \binom{K-1}{m}$ distinct election events in a $K$-candidate plurality contest, each with a unique mapping from a single ballot to the outcome.  

<!-- XXX TODO: Show election events in table  -->

[^no_ties]: Recall that, because $\mathbf{v}$ is continuous, there is always a candidate with the highest vote share. 

In other positional methods such as Borda count the enumeration of election events is more complex. As in plurality, the winner depends on which candidate has the highest score, but now lower rankings may count as well, to differing degrees depending on the rules.[^antiplurality] This means that in principle we need to distinguish more finely among near-ties: for example, we want to distinguish between cases where the winner would change from $a$ to $b$ if the voter ranked $b$ *one* place above $a$ from cases where the winner would change from $a$ to $b$ if the voter ranked $b$ *two* places above $a$. To anticipate the direction we will take below, in such cases the analysis becomes simpler if we assume that e.g. the probability of $b$ finishing with a score no more than $\varepsilon$ below $a$ is twice the probability of $b$ finishing with a score no more than $\varepsilon/2$ below $a$.   

[^antiplurality]: In anti-plurality all rankings count for 1 except for the lowest ranking. Analysis is thus the exact reverse of plurality, where the winner is the candidate with the fewest lowest rankings.  

In other methods such as instant-runoff voting and Kemeny-Young (a Condorcet method), a reasonable strategy for enumerating the relevant election events is to begin with the conditions under which a candidate wins and begin reversing inequalities to discover ways in which a winner may become a loser.  


### Computing nearly-exact event probabilities 

Given a set of inequalities defining election event $E$ and an arbitrary distribution over election outcomes $f(\mathbf{v})$, we can use tools from computational geometry to compute the event probability $\pi_{E} = \int_E f(\mathbf{v}) \, d \mathbf{v}$ with arbitrary precision.

The first step is to determine the vertices that lie at the corners of the subspace defined by the election event; that is, we convert the set of inequalities defining $E$ into a set of vertices defining $E$'s convex hull. (We use the `rcdd::scdd()` function in `R`, which is an implementation of the C library `cddlib`.) Next we apply Delaunay triangulation to divide the region defined by $E$ into disjoint simplices. For example, if $B=3$ we divide $E$ into triangles; if $B = 4$ we divides $E$ into tetrahedrons. (We use the `geometry::delaunayn()` function in `R`, which interfaces with the Qhull library: TODO cite.)  Finally, the adaptive integration algorithm by Genz and Cools (TODO: cite) computes the integral of the election distribution function for each simplex. The algorithm efficiently allocates computational power by using a finer mesh (i.e. computing the density at more points) on simplices where the integrand is less flat. (We implement this using the `SimplicialCubature` package by John P. Nolan.) 

```{r implementing_it_3, cache = T}
# change f
alpha3 <- c(10, 7, 5)
expand_grid(n = 1000, tol = c(.1, .01, .001), skip_non_pivot_events = c(F, T), drop_dimension = c(F,T), merge_adjacent_pivot_events = c(F,T)) %>% 
  mutate(pep = pmap(., plurality_event_probabilities, alpha = alpha3)) -> results_df
```

```{r check_P}
P_mat_from_eppp(results_df$pep[[1]])
apply(P_mat_from_eppp(results_df$pep[[1]]), 2, sum)
P_mat_from_eppp(results_df$pep[[nrow(results_df)]])
apply(P_mat_from_eppp(results_df$pep[[nrow(results_df)]]), 2, sum)
```

```{r checking_it_3}
# total times 
results_df %>% 
  mutate(time = map(pep, "total")) %>% 
  mutate(time_elapsed = map(time, "seconds_elapsed")) %>% 
  unnest(time_elapsed) %>% 
  select(-n, -time, -pep)
...
results_df %>% 
  filter(tol == .01) %>% 
  unnest_longer(pep) %>% 
  mutate(time_elapsed = map(pep, "seconds_elapsed")) %>% 
  unnest(time_elapsed) %>% 
  mutate(type = paste0(skip_non_pivot_events, merge_adjacent_pivot_events, drop_dimension)) %>% 
  select(type, event = pep_id, time_elapsed) %>% 
  pivot_wider(names_from = type, values_from = time_elapsed)

# comparing values 
results_df %>% 
  filter(tol == .01) %>% 
  unnest_longer(pep) %>% 
  mutate(pp = map(pep, "integral")) %>% 
  unnest(pp) %>% 
  mutate(type = paste0(skip_non_pivot_events, merge_adjacent_pivot_events, drop_dimension)) %>% 
  select(type, event = pep_id, pp) %>% 
  pivot_wider(names_from = type, values_from = pp)

```

```{r implementing_it_4, cache = T}
# change f
alpha4 <- c(15, 13, 7, 5)
expand_grid(n = 1000, tol = c(.1, .01, .001), skip_non_pivot_events = c(F, T), drop_dimension = c(F,T), merge_adjacent_pivot_events = c(F,T)) %>% 
  mutate(pep = pmap(., plurality_event_probabilities, alpha = alpha4)) -> results_df4
```
...
```{r get_answer, cache = T}
alpha <- c(11, 10, 7, 3)
answer <- exact_plurality_pivot_probabilities(alpha = alpha, tol = .000001, maxEvals = 1000000)
```

```{r check_times_sc, cache = T}
# Now we want to check the answer we get at various levels of tol 
tibble(tol = c(.1, .01, .001, .0001)) %>% 
  mutate(answers = pmap(., exact_plurality_pivot_probabilities, alpha = alpha, maxEvals = 100000)) -> df
```
...
df %>% 
  ...
  filter(answers_id == "seconds_elapsed") %>% 
  unnest(answers)

```

Okay, next programming tasks here: 

- code up getting all of these via simulation; same approach as the SC one 
- code up getting RMSE relative to `answer` -- I definitely have code somewhere for this. 
- code up SC at lines at 1/2n; lines at 0. 

### Approximations for more efficient computation 

#### Adjacent pivot probs equal 
#### Dropping a dimension
#### Exploiting features of the Dirichlet

Eggers-Nowacki does all three of above for IRV.

#### Independence assumption 

Eggers-Vivyan does this plus all three above for $K$-candidate plurality.


# Rest is previous work.
