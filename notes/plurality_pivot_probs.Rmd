...
An alternative way to estimate $n \pi_{12}$ is to lay out a regular grid of points on the locus where 1 and 2 tie for first, evaluate $f(\mathbf{v})$ at each point in the grid, sum the values, and multiply by the area or volume surrounding each grid point. 

A simple way to implement this approach is to create a regular grid covering the entire unit simplex ($x_1, x_2, \ldots, x_k$ such that $0 \leq x_i \leq 1 \, \forall i$ and $\sum_i x_i = 1$) and then extract the grid points where $x_1 = x_2$ and $x_2 > x_i \, \forall \, i > 2$. 

Below I show grid points for the 3-candidate and 4-candidate cases, with the points highlighted to indicate the density. 

```{r show_grid_approach}
# show it for 3 and 4 side-by-side
ptg3 <- plurality_tie_grid(k = 3, increment = .01)
...
as_tibble(ptg3) %>% 
    mutate(x = V0 + .5*V1,
         y = sqrt(3/4)*V1,
         dens = gtools::ddirichlet(ptg3, alpha = alpha)) %>% 
  ggplot(aes(x = x, y = y)) +
  geom_point(aes(col = dens), size = 1, show.legend = F) + 
  coord_fixed() + 
  theme_void() + 
  expand_limits(x = c(-.1, 1.1), y = sqrt(3/4)*c(-.1, 1.1)) + 
  annotate(geom = "text", x = c(1,.5,0), y = c(0,sqrt(3/4),0) + .05*c(-1,1,-1), label = c("a", "b", "c")) + 
...
# for the other one I think I need rgl or plotly.
# this is not quite working. 

library(plotly)

ptg4 <- plurality_tie_grid(k = 4)
...
as_tibble(ptg4) %>% 
    mutate(x = V0 + .5*V1,
         y = sqrt(3/4)*V1,
         z = sqrt(3/4)*last,
         dens = gtools::ddirichlet(ptg4, alpha = alpha4),
         text = paste("x = ", V0, sep = "")) %>%
  plot_ly(x = ~x, y = ~y, z = ~z, color = ~dens, hoverinfo = "text") %>% 
  layout(xaxis = list(showticklabels = F), yaxis = list(showticklabels = F), zaxis = list(showticklabels = F)) 
...
The simplicial cubature approach extends and improves on the grid-based approach. 
...
- write up grid and SC approaches -- can visualize a nice picture, but how to make? 
